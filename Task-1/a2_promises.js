...
function getUserCB(id, cb) { //моковый запрос пользователя
    setTimeout(() => cb(null, { id, name: "User" + id }), 200);    // cb - callback(error, data)
}
...
function getOrdersCB(userId, cb) {
    setTimeout(() => {
        ...
        if (chance < 0.4) return cb(new Error("DB failure")); // 0.1 шанс что база упадет типо
        cb(null, ["o1", "o2"]);
    }, 250);
}

function getRecsCB(userId, cb) { // да б**ть че такое Recs
...
function runSequential() {
    ...
    return getUserP(1)
        ...
        .then(({ user, orders }) => {
            ...
            if(orders.length === 0) {
                throw new Error("Error, There is no orders");
            }
...
function runParallel() {
    ...
    return getUserP(2)
        .then(user => {
            ...
            return Promise.all(
                [getOrdersP(user.id), getRecsP(user.id)]
