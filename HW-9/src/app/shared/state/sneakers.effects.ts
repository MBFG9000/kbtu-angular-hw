...
export class SneakersEffects {
  ...
  loadSneakers$ = createEffect(() =>
    this.actions$.pipe( //Actions — это RxJS Observable, который непрерывно «льёт поток» всех экшенов, которые происходят в приложении.
      ofType(SneakersActions.loadSneakers), // слушаем этот action ofType — это фильтр по типу экшнов.
      switchMap(({ query }) => { //отменяет предыдущий запрос, если пришёл новый
